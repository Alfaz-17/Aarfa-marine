...
export default function AdminBrandPage() {
  ...
  if (isLoading) return <div className="text-xs font-mono font-bold uppercase tracking-widest animate-pulse text-[#5B9BD5]">Syncing Brand Registry...</div>;
  ...
  return (
    <div className="max-w-4xl space-y-12 font-sans">
      ...
      <div className="border-b border-slate-800 pb-8 relative">
        ...
        <p className="text-xs font-mono font-bold text-[#5B9BD5] uppercase tracking-[0.3em] mt-2 m-0">Manage Manufacturer Relationships & Branding</p>
      </div>
      ...
      <div className="bg-[#0A1F40]/55 p-10 border border-[#5B9BD5]/20 relative">
         ...
         <div className="flex justify-between items-center mb-8">
            <h2 className="text-sm font-mono font-bold uppercase tracking-widest text-[#5B9BD5] m-0">{editingId ? 'Modify Strategic Partner' : 'Onboard New Partner'}</h2>
            {editingId && (
         ...
         <form onSubmit={handleAddBrand} className="space-y-6">
            <div className="flex flex-col md:flex-row gap-6 md:items-end">
               <div className="flex-1 space-y-2">
                  ...
                  <input 
                    ...
                    className="w-full px-6 py-4 bg-slate-950/60 border border-[#5B9BD5]/20 focus:border-[#5B9BD5] focus:bg-slate-950 outline-none text-xs text-white font-mono"
                    value={newBrand.name}
               ...
               <div className="w-full md:w-48 space-y-2 text-center">
                  ...
                  {logoPreview ? (
                     <div className="relative h-14 border border-[#5B9BD5]/20 bg-slate-950 flex items-center justify-center p-2">
                     ...
                     <label className="block h-14 border-2 border-dashed border-slate-800 hover:border-[#5B9BD5] flex items-center justify-center cursor-pointer transition-colors bg-slate-950/40">
            ...
            <button type="submit" disabled={isUploading} className="w-full py-4 bg-[#1E5FA6] hover:bg-[#5B9BD5] text-white text-[10px] font-mono font-bold uppercase tracking-widest border-0 cursor-pointer">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
         {brands.map(brand => (
            <div key={brand._id} className="bg-[#0A1F40]/55 p-6 border border-[#5B9BD5]/20 flex flex-col items-center group relative h-48">
               ...
               <h3 className="text-[10px] font-mono font-bold text-[#5B9BD5] uppercase tracking-widest text-center border-t border-slate-850 pt-4 w-full m-0">{brand.name}</h3>
               <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => handleEdit(brand)} className="p-2 bg-slate-950/80 rounded-full shadow-md text-[#5B9BD5] hover:text-white border-0 cursor-pointer">
